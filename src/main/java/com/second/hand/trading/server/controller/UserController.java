...
//这段代码是一个UserController类，用于处理用户相关的请求。类中包含了注册账号、登录、退出登录、获取用户信息、修改用户公开信息和修改密码等功能的方法。
//
//        1. signIn方法用于用户注册账号，接收一个UserModel对象作为参数，并将用户信息存入数据库。如果注册成功则返回成功的ResultVo对象，否则返回失败的ResultVo对象。
//
//        2. login方法用于用户登录，接收账号和密码作为参数，并校验用户信息是否正确。如果登录成功则返回成功的ResultVo对象，并设置cookie，否则返回失败的ResultVo对象。
//
//        3. logout方法用于用户退出登录，接收用户id并清除cookie，返回成功的ResultVo对象。
//
//        4. getOneUser方法用于获取用户信息，接收用户id并返回用户信息的ResultVo对象。
//
//        5. updateUserPublicInfo方法用于修改用户公开信息，接收用户id和UserModel对象，并更新用户信息，返回成功的ResultVo对象。
//
//        6. updateUserPassword方法用于修改用户密码，接收用户id、旧密码和新密码作为参数，并更新密码，返回成功的ResultVo对象。
//
//        整个类的功能主要是围绕用户的注册、登录、信息获取和修改等操作展开，并且通过注解配置了接口的访问路径和参数校验等信息。
